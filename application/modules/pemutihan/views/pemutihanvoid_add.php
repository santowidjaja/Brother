...
<li><?= $title; ?></li>
 ...
 <div class="box">
      <div class="box-header with-border">
        <h3 class="box-title">Informasi Pemutihan</h3>
      </div>
      <div class="box-body"></div>
        <div class="row">
          <div class="col-md-6">
          <form action="<?php base_url('pemutihan/pemutihanvoid_add') ?>" method="post">
              <div class="form-group">
                <label for="nomor_nota">No.Nota Pemutihan</label><br>
                   
                <input class="form-control" type="text" id="nomor_nota" name="nomor_nota" value="<?= $getpemutihanmaster['nomor_nota'] ?> "readonly/>             
              </div>
              ...
              <div class="form-group <?php echo form_error('namasiswa') ? 'has-error' : '' ?>">
                ...
                <input type='text' name='namasiswa' class='form-control' id='namasiswa' value="<?= $getpemutihanmaster['namasiswa'] ?>"readonly>
              ...
              <div class="form-group">
                <input type="hidden" name="siswa_id" class="form-control" id="siswa_id"  value="<?= $getpemutihanmaster['siswa_id'] ?>"/>
              ...
              <a href="<?= base_url('pemutihan/pemutihanvoid_add/'.$id_master); ?> " class="btn btn-default">Cancel</a>
              <a href="<?= base_url('pemutihan/pemutihanvoid_list'); ?> " class="btn btn-warning">Kembali</a>
                </div>
            </form>         
                </div>
<b>Detail Biaya</b>
...
<table class="table">
  ...
  <tbody>
  <?php $no='1'; ?>
  <?php foreach ($getpemutihandetail as $item): ?>
    <tr>
      <th scope="row"><?= $no ?></th>
      <td><?= $item['biaya'] ?></td>
