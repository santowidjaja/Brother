    ...
    <?php if($getlistsiswa){ ?>
        ...
        <table id='example' class='table table-bordered table-striped'>
            <thead>
                ...
                <th><center>Kegiatan Extrakulikuler</center></th>
...
<form method="post" action="<?= base_url('guru/extrakulikuler_update/'.$get_extraedit['id']) ?>" >
<?php }else{ ?>
<form method="post" action="<?= base_url('guru/extrakulikuler_add') ?>" >
<?php }?>
<tr><td><?= $sno++ ?></td>
<td><?= $dt['nis']?></td>
<td><?= $dt['namasiswa']?> </td>
...
<td><center><input type='number' name='b1' value='<?= $get_extraedit['nilai']?>' style='width:70px; text-align:center; padding:0px; color:blue'></center></td>
...
<td><center><input type='number' name='b1' value='' style='width:70px; text-align:center; padding:0px; color:blue'></center></td>
...
<td></td>
<td><?= $dt2['kegiatan']?> </td>
<td><center><?= $dt2['nilai']?></center></td>
<td><?= $dt2['deskripsi']?></td>
<td>
<a href="<?= base_url('guru/extrakulikuler_edit/'.$tahunakademik_id.'/'.$kelas_id.'/'.$dt2['extra_id'])?>"class='btn btn-xs btn-warning'><span class='glyphicon glyphicon-edit'></span></a>
    <a href="<?= base_url('guru/extrakulikuler_hapus/'.$tahunakademik_id.'/'.$kelas_id.'/'.$dt2['extra_id'])?>"class='btn btn-xs btn-danger' onclick="return confirm('Apa anda yakin untuk hapus Data ini?')"><span class='glyphicon glyphicon-remove'></span></a>
    </td>
</tr>
<?php } 
endforeach; ?>
