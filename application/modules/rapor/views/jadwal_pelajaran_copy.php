  ...
  <section class="content">
    ...
    <div class="box">
      <div class="box-header with-border">
        ...
        <a href="<?= base_url('rapor/jadwal_pelajaran'); ?>" class="btn btn-warning">
        Kembali</a>
    </a>
</form>
        </div>
...
<table class='table table-hover'>
        ...
        <form   class="form-inline" method="post" action="<?= base_url('rapor/jadwal_pelajaran_copy_add')?>" enctype ="multipart/form-data" id="posts">
        <select name="tahunakademik_tujuan" class="form-control <?= form_error('tahunakademik_tujuan') ? 'is-invalid' : '' ?>">
                            <?php foreach ($tahunakademik_genap as $dt) : ?>
                            <option value="<?= $dt['id']; ?>"<?= set_select('tahunakademik_id', $dt['id'], FALSE); ?>><?= $dt['nama']; ?></option>
                            <?php endforeach; ?>
                        </select>
        <input type="hidden" value="<?= $tahunakademik_asal ?>" name="tahunakademik_asal">
        <input type="hidden" value="<?= $kelas_tujuan ?>" name="kelas_tujuan">
        <input type="submit" value="Duplicate" name="submit"class="btn btn-warning"onclick="return confirm('Anda yakin ? data akan replace, dan tidak dapat dikembalikan lagi...');">
