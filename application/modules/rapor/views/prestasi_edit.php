...
<form method="post" action="<?= base_url('rapor/prestasi_update/'.$get_prestasiedit['id']) ?>" >
<?php }else{ ?>
<form method="post" action="<?= base_url('rapor/prestasi_add') ?>" >
<?php }?>
<tr><td><?= $sno++ ?></td>
<td><?= $dt['nis']?></td>
<td><?= $dt['namasiswa']?> </td>
...
<td>
<a href="<?= base_url('rapor/prestasi_edit/'.$tahunakademik_id.'/'.$kelas_id.'/'.$dt2['prestasi_id'])?>"class='btn btn-xs btn-warning'><span class='glyphicon glyphicon-edit'></span></a>
    <a href="<?= base_url('rapor/prestasi_hapus/'.$tahunakademik_id.'/'.$kelas_id.'/'.$dt2['prestasi_id'])?>"class='btn btn-xs btn-danger' onclick="return confirm('Apa anda yakin untuk hapus Data ini?')"><span class='glyphicon glyphicon-remove'></span></a>
    </td>
</tr>
<?php } 
endforeach; ?>
